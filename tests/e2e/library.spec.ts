...
test('XSS fixtures render as inert text everywhere (§28)', async ({ page }) => {
	await page.addInitScript(() => {
		window.addEventListener('error', (event) => {
			if ((event as ErrorEvent).message?.includes('alert'))
				(window as { __xssFired?: boolean }).__xssFired = true;
		});
	});
	...
	expect(
		await page.evaluate(() => (window as { __xssFired?: boolean }).__xssFired ?? false)
	).toBeFalsy();
});
